...
)
{
  ...
  pTDIH_DeviceExtension = (PTDIH_DeviceExtension )(Context);
   ...
   case IRP_MJ_CREATE:
	   ...
	   strcpy(event.addr3,"*");
	   strcpy(event.addr4,"*");
	   strcpy(event.RemotePort,"*");
	   strcpy(event.LocalPort,"*");
   ...
   case IRP_MJ_CLOSE:
	   ...
	   else
	   {
		   ...
		   {					
			   ...
			   //端口号					  
			   sprintf(event.RemotePort,"%d",pFileObjectNode->RemotePort);
			   sprintf(event.LocalPort,"%d",pFileObjectNode->LocalPort);
		   }
		   ...
		   case TDI_CONNECT:
			   ...
			   else
			   {
				   ...
				   if(pTransAddr->Address[0 ].AddressType==TDI_ADDRESS_TYPE_IP)
				   {
		               ...
		               if(pTDIH_DeviceExtension->NodeType == TDIH_NODE_TYPE_TCP_FILTER_DEVICE)				  
					   {					
			               strcpy(event.Operation,"TCP---CONNECT");			
					   }				  
		               else if(pTDIH_DeviceExtension->NodeType == TDIH_NODE_TYPE_UDP_FILTER_DEVICE)				   
					   {					
			               strcpy(event.Operation,"UDP---CONNECT");			
					   }			
		               else				
					   {					  
			               strcpy(event.Operation,"OTHER---CONNECT");				  
					   }
			           ...
			           addr3=(int)pByte[2];
			           addr4=(int)pByte[3];
					   RemotePort=LLT_htons(pIPAddress->sin_port);
			      
			           sprintf(event.RemotePort,"%d",RemotePort);
					   ...
					   {				       
		                   ...
		                   LocalPort=LLT_htons(pIPAddress->sin_port);
						   sprintf(event.LocalPort,"%d",LocalPort);
					   }                  		
					   ...
					   pFileObjectNode->RemotePort=RemotePort;
                       pFileObjectNode->LocalPort=LocalPort;
...
)
{
   ...
   if( !NT_SUCCESS(status) )
   {
      DbgPrint("Couldn't create the TCP Filter Device Object\n");
   ...
   {
      DbgPrint("Couldn't attach to TCP Device Object\n");
...
)
{
   ...
   DbgPrint("UDPFilter_Attach Entry...\n");
   ...
   if( !NT_SUCCESS(status) )
   {
      DbgPrint("Couldn't get the UDP Device Object\n");
   ...
   if( !NT_SUCCESS(status) )
   {
      DbgPrint("Couldn't create the UDP Filter Device Object\n");
...
//      ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
   ...
   if( !pLowerDeviceObject )
   {
      DbgPrint("Couldn't attach to UDP Device Object\n");
...
)
{
   ...
   try
   {
      try
      {
        ...
        DbgPrint("TCPFilter_Detach Finished\n");
	}
...
)
{
   ...
   try
   {
      try
      {
        ...
        DbgPrint("UDPFilter_Detach Finished\n");
	}
    except (EXCEPTION_EXECUTE_HANDLER)
    {
...
void DebugPrintMsg(PEVENT event)
{
	...
	RemotePortLen=ANSIstrlen(event->RemotePort)+1;
	LocalPortLen=ANSIstrlen(event->LocalPort)+1;
	ResultLen=ANSIstrlen(event->SuccOrFail)+1;

	EventDataLen=TimeLen+ProcessIdLen+ProcessNameLen+addr1Len+addr2Len+addr3Len+addr4Len+OperationLen+RemotePortLen+LocalPortLen+ResultLen;
	...
	if(pEvent!=NULL)
	{
		...
		buffer+=OperationLen;
		RtlCopyMemory(buffer,event->RemotePort,RemotePortLen);
		buffer+=RemotePortLen;
        RtlCopyMemory(buffer,event->LocalPort,LocalPortLen);
		buffer+=LocalPortLen;
...
void DebugPrintSystemThread(IN PVOID Context)
{
	...
	RtlInitUnicodeString(&DebugPrintName,L"\\Device\\PHDDebugPrint1");
   ...
   )
{
   ...
   if( !NT_SUCCESS(Status) )
   {
      DbgPrint("IoCallDriver(pDeviceObject = %lx) returned %lx\n",pDeviceObject,Status);
   }
   ...
   if ((Status == STATUS_PENDING) || (Status == STATUS_SUCCESS))
   {
      ...
      if (!NT_SUCCESS(Status))
      {
         DbgPrint("LLT_MakeSimpleTdiRequest could not wait Wait returned %lx\n",Status);
         return Status;
      }
