...
NTSTATUS DbpCreate(	IN PDEVICE_OBJECT fdo,
					IN PIRP Irp)
{
	DbgPrint("NENG CHUANG JIAN QUE BU NENG XIE");
...
{
	DbgPrint("xiebuxie?");
	DbgPrint("wojiubuxinzhegexiecaozuodoujinbuqu,niquchishiba");
	DbgPrint("renduoshizhongwoyaonideming");
	PDEBUGPRINT_DEVICE_EXTENSION dx = (PDEBUGPRINT_DEVICE_EXTENSION)fdo->DeviceExtension;
	if( dx->IODisabled)
		return CompleteIrp( Irp, STATUS_DEVICE_NOT_CONNECTED, 0);
	if( !LockDevice(dx))
...
/*	LARGE_INTEGER Now;
	...
	ULONG EventLen=strlen(event.ProcessName)+1;
	ULONG EventDataLen = sizeof(TIME_FIELDS) + EventLen;
	...
	RtlCopyMemory( buffer, event.ProcessName, EventLen);
