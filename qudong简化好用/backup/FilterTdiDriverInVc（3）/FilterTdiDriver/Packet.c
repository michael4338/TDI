		   ...
		   case TDI_CONNECT:
			   ...
			   else
			   {
				   ...
				   if(pTransAddr->Address[0 ].AddressType==TDI_ADDRESS_TYPE_IP)
				   {
					   ...
					   pFileObjectNode->a=20480;
					   pFileObjectNode->b=100;
			   ...
			   else
			   {
				   ...
				   else
				   {
				       ...
				       */
					   ...
					   //IP地址
					   sprintf(event.addr1, "%d" , pFileObjectNode->a);
			           sprintf(event.addr2, "%d" , pFileObjectNode->a);
			           sprintf(event.addr3, "%d" , pFileObjectNode->a);
			           sprintf(event.addr4, "%d" , pFileObjectNode->a);
                       

					   //端口号
					   sprintf(event.port,"%d",pFileObjectNode->b);
