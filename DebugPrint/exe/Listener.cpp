...
UINT ListenThreadFunction( LPVOID pParam)
{
	...
	//创建文件句柄和读文件写文件应该在一个线程之内
	ListenerInfo.DebugPrintDriver = CreateFile("\\\\.\\PHDDebugPrint2", 
	...
	if( ListenerInfo.DebugPrintDriver != INVALID_HANDLE_VALUE ) 
	{ 
        AfxMessageBox( "创建设备成功" ); 
	} 
    else 
	{ 
	...
	DWORD TypeBytesWrite;
	
	char EventWrite[EventWriteLen+1];
	memset(EventWrite,0,EventWriteLen+1);
    EventWrite[0]='S';

	if(WriteFile( ListenerInfo.DebugPrintDriver, EventWrite, EventWriteLen,&TypeBytesWrite,&ol))
		AfxMessageBox("WRITING START SUCCESSFULLY");
	for(;;)
	{
		DWORD TxdBytes;
		...
		//远程端口号
        char* pRemotePort=Event+len;
		len+=strlen(pRemotePort)+1;

		//本地端口号
		char* pLocalPort=Event+len;
		len+=strlen(pLocalPort)+1;

		//结果是否成功
		char *result=Event+len;
...
Exit:
	EventWrite[0]='T';
    if(WriteFile( ListenerInfo.DebugPrintDriver, EventWrite, EventWriteLen,&TypeBytesWrite,&ol))
		AfxMessageBox("WRITING STOP SUCCESSFULLY");
